...
import {
  ...
  questionRemoveHelper, questionCreateHelper
} from './testHelper';
beforeEach(() => {
  clearHelper();
});
...
describe('No data', () => {
  test('Failure-no data', () => {
    const result = questionRemoveHelper(1, 1, '1');
...
describe('With Data', () => {
  ...
  test('Invalid quiz ID', () => {
    const result = questionRemoveHelper(quizzes[0] - 1, 1, users[0]);
  ...
  test('Unauthorised user', () => {
    const result = questionRemoveHelper(quizzes[0], 1, users[0] + '1');
    expect(result).toEqual({
      error: 'UNAUTHORISED',
      message: expect.any(String)
    });
  ...
  test('Invalid question ID', () => {
    const result = questionRemoveHelper(quizzes[0], 1, users[0]);
    expect(result).toEqual({
  ...
  describe('Single Question', () => {
    ...
    test('Invalid Question', () => {
      const result = questionRemoveHelper(quizzes[0], question[0] + 1, users[0]);
      expect(result).toEqual({
        error: 'INVALID_QUESTION_ID',
        message: expect.any(String)
      });
    ...
    test('Success', () => {
      const result = questionRemoveHelper(quizzes[0], question[0], users[0]);
      expect(result).toEqual({});
