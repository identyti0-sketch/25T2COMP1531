...
describe('gameStart Tests', () => {
  ...
  describe(('With questions'), () => {
    ...
    test('Successful game start', () => {
      const success: object = gameStartHelper(quizzes[0], users[0].session, 5);
      expect(success).toStrictEqual({ gameId: expect.any(Number) });
      // const game = gameStatusHelper(users[0].session, quizzes[0]);
